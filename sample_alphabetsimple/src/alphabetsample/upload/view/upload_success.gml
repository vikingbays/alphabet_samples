...
<body>
  ...
  <div class="bs-callout bs-callout-danger" id="callout-glyphicons-dont-mix">
       ...
       <div class="respDataForName">
       		名称 : <span>{{.Datas.Name}}</span>
       </div>
       <div class="respDataForSummary">
       		摘要 : <span>{{.Datas.Summary}}</span>
       </div>
       <div class="respDataForType">
       ...
       <div style='width:100%;'>
        ...
        <div style='width:660px;clear:both'>
          <div class="respDataForAlias" style='width:130px;float:left'>{{$v.Alias}}</div>
          <div class="respDataForAuthor" style='width:130px;float:left'>{{$v.Author }}</div>
          <div class="respDataForPath" style='width:400px;float:left;word-break:break-all; '>{{$v.Path}}</div>
        </div>
          	{{end}}
          	{{end}}
