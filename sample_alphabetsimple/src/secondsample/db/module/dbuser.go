...
func ExistUser1Table() bool {
	dataFinder, err := sqler.GetConnectionForDataFinder("db_test2")
...
//新增数据
func InsertUser1WithPtr(userList []*User1) bool {
	dataFinder, err := sqler.GetConnectionForDataFinder("db_test2")
...
func InsertUser1(userList []User1) bool {
	dataFinder, err := sqler.GetConnectionForDataFinder("db_test2")
	defer dataFinder.ReleaseConnection(1)
	if err == nil {
		dataFinder.StartTrans()
...
//查询数据采用指针
func SelectUser1ListWithPtr(minUserId, maxUserId int) []User1 {
	dataFinder, err := sqler.GetConnectionForDataFinder("db_test2")
	defer dataFinder.ReleaseConnection(0)
	if err == nil {
		...
		userInterfaceList := make([]User1, 0, 1)
...
//查询数据采用指针
func SelectUser1ListWithPtr2(minUserId, maxUserId int) []User1 {
	dataFinder, err := sqler.GetConnectionForDataFinder("db_test2")
	defer dataFinder.ReleaseConnection(0)
	if err == nil {
		//paramUser := User1{MinUsrid: minUserId, MaxUsrid: maxUserId}
...
func SelectUser1MapWithPtr(minUserId, maxUserId int) map[string]User1 {
	dataFinder, err := sqler.GetConnectionForDataFinder("db_test2")
	defer dataFinder.ReleaseConnection(0)
	if err == nil {
		//paramUser := User1{MinUsrid: minUserId, MaxUsrid: maxUserId}
...
//查询数据
func SelectUser1List(minUserId, maxUserId int) []User1 {
	dataFinder, err := sqler.GetConnectionForDataFinder("db_test2")
	defer dataFinder.ReleaseConnection(0)
	if err == nil {
		//paramUser := User1{MinUsrid: minUserId, MaxUsrid: maxUserId}
...
func SelectUser1Map(minUserId, maxUserId int) map[string]User1 {
	dataFinder, err := sqler.GetConnectionForDataFinder("db_test2")
	defer dataFinder.ReleaseConnection(0)
	if err == nil {
		//paramUser := User1{MinUsrid: minUserId, MaxUsrid: maxUserId}
