...
{{ IncludeHTML "frame/subheader" "submenu=5&active=3"  nil  .Header  "POST" }}

  <div>
  <div class="bs-callout bs-callout-danger" id="callout-glyphicons-dont-mix">
    <h4>输出Session信息</h4>
    <span>城市：</span><span class="respDatasForCitySecond">{{.Session.session_second_city}}</span>
    <span>性别：</span><span class="respDatasForSexSecond">{{.Session.session_second_sex}}</span>
    <span>年龄：</span><span class="respDatasForAgeSecond">{{.Session.session_second_Age}}</span>
  </div>
  </div>
</body>
</html>
