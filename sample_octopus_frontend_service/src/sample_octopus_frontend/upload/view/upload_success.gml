...
<body>
  ...
  <div class="bs-callout bs-callout-danger" id="callout-glyphicons-dont-mix">
       ...
       <div class="respDataForNameOctopus">
       		名称 : <span>{{.Datas.Name}}</span>
       </div>
       <div class="respDataForSummaryOctopus">
       		摘要 : <span>{{.Datas.Summary}}</span>
       </div>
       <div class="respDataForTypeOctopus">
       ...
       <div style='width:100%;'>
        ...
        <div style='width:660px;clear:both'> 
          <div class="respDataForAliasOctopus" style='width:130px;float:left'>{{$v.Alias}}</div>
          <div class="respDataForAuthorOctopus" style='width:130px;float:left'>{{$v.Author }}</div>
          <div class="respDataForPathOctopus" style='width:400px;float:left;word-break:break-all; '>{{$v.Path}}</div>
        </div>
          	{{end}}
          	{{end}}
